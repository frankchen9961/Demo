  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        copy_from_doc_version_id:{
          bsonType: "objectId",
          description: "複製來源文件版本ID",
        },
        enable: {
          bsonType: "bool",
          description: "是否啟用",
    ...
    $jsonSchema: {
      ...
      properties: {
        ...
        classification:{
          ...
          description: "資訊分級: internal-內部文件, public-公開文件",          
        },
        audit_info: {
          bsonType: "object",
          required: ["create_time", "creator", "modify_time", "modifier"],
...
// QA Chunks Collection Schema Validation
function createQAChunksCollection(collectionName) {
  try {
    db.getCollection(collectionName).drop();
    print("已刪除現有的 " + collectionName + " collection");
  } catch (e) {
      ...
      $jsonSchema: {
        ...
        properties: {
          ...
          enable_status: {
            ...
            description: "啟用狀態",
            properties: {
              knowledge_base_qa_enable: {
                bsonType: "bool",
                description: "知識庫QA啟用狀態: true-啟用, false-停用",
              },
              knowledge_base_enable: {
                bsonType: "bool",
                description: "知識庫啟用狀態: true-啟用, false-停用",
